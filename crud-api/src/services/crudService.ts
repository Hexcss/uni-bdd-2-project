...
const update = async (req: Request, res: Response, dataType: string) => {
    try {
        ...
        switch(dataType) {
            case "Category": {
                updatedData = await Category.findOneAndUpdate({id}, req.body, {new: true});
                break;
            }
            case "Recipe": {
                updatedData = await Recipe.findOneAndUpdate({id}, req.body, {new: true});
                break;
            }
            case "Tag": {
                updatedData = await Tag.findOneAndUpdate({id}, req.body, {new: true});
                break;
            }
        }
