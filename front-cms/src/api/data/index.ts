...
interface getDataResponse {
  data: ITag[] | ICategory[] | IRecipe[] | null;
  totalCount: number;
}
...
export async function getData(
  ...
  limit: number = 10,
  search: string = ""
): Promise<getDataResponse | null> {
  const encryptedToken = localStorage.getItem("token");
  const authToken = encryptedToken ? decryptToken(encryptedToken) : "";

  try {
    const { data } = await axios.get(`${environment.CRUD_API_URL}/${type}?page=${page}&limit=${limit}&search=${search}`, {
      headers: {
